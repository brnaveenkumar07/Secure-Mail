...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    ...
    // Attempt login without face first
    const result = await login(username, password);

    if (result.success) {
      navigate("/gmail-dashboard");
    } else if (result.requireFaceAuth) {
      // If backend requires face auth, show modal
      setShowFaceID(true);
    } else {
  ...
  const handleFaceVerified = async (faceImage) => {
    // Retry login with face image
    const result = await login(username, password, faceImage);
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center p-4">
        ...
        <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-md">
          <div className="text-center mb-8">
            ...
            <p className="text-gray-600">Face ID & Digital Verification System</p>
          </div>
